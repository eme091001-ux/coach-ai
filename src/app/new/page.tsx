...
import { FeedbackInput, MeetingType } from "@/types";
import { STAFF_LIST } from "@/lib/mockData";
import { Loader2, Sparkles } from "lucide-react";

const MEETING_TYPES: MeetingType[] = ["初回面談", "求人提案", "面接対策", "内定承諾", "法人営業", "商談"];
...
export default function NewFeedbackPage() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // 実際のDBがない間はダッシュボードへ
      alert(`フィードバック生成完了！\n総合スコア: ${data.totalScore}点\n次回テーマ: ${data.nextTheme}\n\n※現在はデモモードのため一覧には反映されません`);
      router.push("/feedback");
    } catch {
      setError("フィードバックの生成に失敗しました。APIキーを確認してください。");
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="grid grid-cols-2 gap-5">
        ...
        <div className="space-y-5">
          <div className="bg-white rounded-xl border border-gray-100 p-5">
            ...
            <div className="space-y-3">
              ...
              <div>
                ...
                <select value={form.staffExperience} onChange={(e) => set("staffExperience", e.target.value)}
                  className="w-full text-sm border border-gray-200 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-emerald-500">
                  {["1年未満", "1〜2年", "3〜5年", "5年以上"].map((v) => <option key={v}>{v}</option>)}
                </select>
              </div>

              <div>
                <label className="text-xs text-gray-500 block mb-1">求職者名 / 企業名</label>
          ...
          <div className="bg-white rounded-xl border border-gray-100 p-5">
            ...
            <div className="space-y-3">
              <div>
                <label className="text-xs text-gray-500 block mb-1">会社の営業方針</label>
                <textarea value={form.companyPolicy} onChange={(e) => set("companyPolicy", e.target.value)}
                  rows={3} className="w-full text-sm border border-gray-200 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-emerald-500 resize-none" />
              </div>
              <div>
                <label className="text-xs text-gray-500 block mb-1">上司の評価軸</label>
                <textarea value={form.managerPolicy} onChange={(e) => set("managerPolicy", e.target.value)}
                  rows={3} className="w-full text-sm border border-gray-200 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-emerald-500 resize-none" />
              </div>
              <div>
                <label className="text-xs text-gray-500 block mb-1">NGワード</label>
                <input type="text" value={form.ngWords} onChange={(e) => set("ngWords", e.target.value)}
                  className="w-full text-sm border border-gray-200 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-emerald-500" />
              </div>
            </div>
          </div>
        ...
        <div className="flex flex-col gap-5">
          <div className="bg-white rounded-xl border border-gray-100 p-5 flex-1">
            <div className="flex items-center justify-between mb-3">
              <h2 className="text-sm font-medium text-gray-900">文字起こしテキスト</h2>
              <button
